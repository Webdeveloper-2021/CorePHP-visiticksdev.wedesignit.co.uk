...
///*****************************************************************************///
	...
	$PHPTemplateLayer->prepare($install_path."/admin/templates/categories.htm");
...
///*****************************************************************************///
	...
	$baseurl = "categories.php?tab=$tab&pagenumber=";
	...
	$requesturl = "categories?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
	$requesttype = "GET";
	$requestbody = "";
	...
	$requestresult = process_connect_Curl($U_SESSION_API_TOKEN,$requesturl,$requesttype,$requestbody,API_URL,API_USERNAME,API_PASSWORD,API_USERAGENT,SETTING_LOGS,$U_DATE,$U_DATETIME,INSTALL_TYPE,INSTALL_VERSION);
		...
		$requestresult = json_decode($requestresultarray[1]);
			...
			$totalresults = count($requestresult);
				...
				if($totalresults > "0")
				{
				$nextrequesturl = "categories?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
				$nextrequesttype = "GET";
				$nextrequestbody = "";
			...
			$PHPTemplateLayer->assignGlobal("pagenumber",$pagenumber);
				...
				$PHPTemplateLayer->assignGlobal("lastCategoryDisplayOrder",$requestresult[$totalresults - 1]->displayOrder);
					...
					$categoryimage = $categoryobject->imageFileName;
					$categoryvisible = $categoryobject->visible;
					$categorydisplayOrder = $categoryobject->displayOrder;
					...
					$PHPTemplateLayer->assign("contentimage",$categorypicture);
					$PHPTemplateLayer->assign("contenttitle",$categorytitle);
					$PHPTemplateLayer->assign("contentstatus",$contentstatus);
					}
				}
				else
				{
