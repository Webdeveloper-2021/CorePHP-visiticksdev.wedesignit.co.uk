...
///*****************************************************************************///
	...
	$PHPTemplateLayer->prepare($install_path."/admin/templates/settings-configuration-donationrates.htm");
...
///*****************************************************************************///
		...
		if($_POST['add'])
		{
		header("Location:settings-configuration-donationrates-add.php");
		exit();
		}
...
///*****************************************************************************///
	...
	$baseurl = "settings-configuration-donationrates.php?tab=$tab&pagenumber=";
	...
	$requesturl = "donationrates?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
	$requesttype = "GET";
	$requestbody = "";
	...
	$requestresult = process_connect_Curl($U_SESSION_API_TOKEN,$requesturl,$requesttype,$requestbody,API_URL,API_USERNAME,API_PASSWORD,API_USERAGENT,SETTING_LOGS,$U_DATE,$U_DATETIME,INSTALL_TYPE,INSTALL_VERSION);
		...
		$requestresult = json_decode($requestresultarray[1]);
			...
			$totalresults = count($requestresult);
				...
				if($totalresults > "0")
				{
				$nextrequesturl = "donationrates?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
				$nextrequesttype = "GET";
				$nextrequestbody = "";
			...
			$PHPTemplateLayer->assignGlobal("pagenumber",$pagenumber);
				...
				usort($requestresult, 'process_content_displayOrder');
					...
					$donationid = $categoryobject->id;
					$donationtitle = $categoryobject->name;
					$donationtype = $categoryobject->type;
					$donationamount = $categoryobject->amount;
					$donationpercentage = $categoryobject->percentage;

						if($donationtype == "1")
						{
						$donationtext = "&pound;".$donationamount;
						}
						elseif($donationtype == "2")	
						{
						$donationtext = $donationpercentage."%";
						}

					$PHPTemplateLayer->assign("contentid",$donationid);
					$PHPTemplateLayer->assign("contenttitle",$donationtitle);
					$PHPTemplateLayer->assign("contentdonation",$donationtext);
					}
				}
				else
				{
