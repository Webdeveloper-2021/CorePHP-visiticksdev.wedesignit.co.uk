...
///*****************************************************************************///
	...
	$PHPTemplateLayer->prepare($install_path."/admin/templates/settings-configuration-taxcodes.htm");
...
///*****************************************************************************///
		...
		if($_POST['add'])
		{
		header("Location:settings-configuration-taxrates-add.php");
		exit();
		}
...
///*****************************************************************************///
	...
	$baseurl = "settings-configuration-taxcodes.php?tab=$tab&pagenumber=";
	...
	$requesturl = "taxcodes?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
	$requesttype = "GET";
	$requestbody = "";
	...
	$requestresult = process_connect_Curl($U_SESSION_API_TOKEN,$requesturl,$requesttype,$requestbody,API_URL,API_USERNAME,API_PASSWORD,API_USERAGENT,SETTING_LOGS,$U_DATE,$U_DATETIME,INSTALL_TYPE,INSTALL_VERSION);
		...
		$requestresult = json_decode($requestresultarray[1]);
			...
			$totalresults = count($requestresult);
				...
				if($totalresults > "0")
				{
				$nextrequesturl = "taxcodes?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
				$nextrequesttype = "GET";
				$nextrequestbody = "";
			...
			$PHPTemplateLayer->assignGlobal("pagenumber",$pagenumber);
				...
				usort($requestresult, 'process_content_displayOrder');
					...
					$taxid = $categoryobject->id;
					$taxtitle = $categoryobject->name;
					$taxcode = $categoryobject->posCode;
					$taxrate = $categoryobject->rate;

					$PHPTemplateLayer->assign("contentid",$taxid);
					$PHPTemplateLayer->assign("contenttitle",$taxtitle);
					$PHPTemplateLayer->assign("contentcode",$taxcode);
					$PHPTemplateLayer->assign("contentrate",$taxrate);
					}
				}
				else
				{
