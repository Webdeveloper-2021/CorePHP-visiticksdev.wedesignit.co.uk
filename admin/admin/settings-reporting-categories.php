...
///*****************************************************************************///
	...
	$PHPTemplateLayer->prepare($install_path."/admin/templates/settings-reporting-categories.htm");
...
///*****************************************************************************///
		...
		if($_POST['add'])
		{
		header("Location:settings-reporting-categories-add.php");
		exit();
		}
...
///*****************************************************************************///
	...
	$baseurl = "settings-reporting-categories.php?tab=$tab&pagenumber=";
	...
	$requesturl = "reportcategories?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
	$requesttype = "GET";
	$requestbody = "";
	...
	$requestresult = process_connect_Curl($U_SESSION_API_TOKEN,$requesturl,$requesttype,$requestbody,API_URL,API_USERNAME,API_PASSWORD,API_USERAGENT,SETTING_LOGS,$U_DATE,$U_DATETIME,INSTALL_TYPE,INSTALL_VERSION);
		...
		$requestresult = json_decode($requestresultarray[1]);
			...
			$totalresults = count($requestresult);
				...
				if($totalresults > "0")
				{
				$nextrequesturl = "reportcategories?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
				$nextrequesttype = "GET";
				$nextrequestbody = "";
			...
			$PHPTemplateLayer->assignGlobal("pagenumber",$pagenumber);
				...
				$itemcount = 0;
					...
					$categorygroupid = $categoryobject->reportCategoryGroupId;

						if($categorygroupid)
						{
							$requesturl = "reportcategorygroups/".$categorygroupid;
							$requesttype = "GET";
							$requestbody = "";
							...
							if($requestresult != "ERROR")
							{
	    						...
	    						{
									$categorygrouptitle = $requestresult->name;
								}
								else
								{
									$apierror = 1;
					...
					$PHPTemplateLayer->assign("contentid",$categoryid);
					$PHPTemplateLayer->assign("contenttitle",$categorytitle);
					$PHPTemplateLayer->assign("contentgrouptitle",$categorygrouptitle);
					}
				}
				else
				{
