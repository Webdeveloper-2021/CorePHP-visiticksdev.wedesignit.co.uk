...
///*****************************************************************************///
	...
	$PHPTemplateLayer->prepare($install_path."/admin/templates/settings-reporting-groups.htm");
...
///*****************************************************************************///
		...
		if($_POST['add'])
		{
		header("Location:settings-reporting-groups-add.php");
		exit();
		}
...
///*****************************************************************************///
	...
	$baseurl = "settings-reporting-groups.php?tab=$tab&pagenumber=";
	...
	$requesturl = "reportcategorygroups?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
	$requesttype = "GET";
	$requestbody = "";
	...
	$requestresult = process_connect_Curl($U_SESSION_API_TOKEN,$requesturl,$requesttype,$requestbody,API_URL,API_USERNAME,API_PASSWORD,API_USERAGENT,SETTING_LOGS,$U_DATE,$U_DATETIME,INSTALL_TYPE,INSTALL_VERSION);
		...
		$requestresult = json_decode($requestresultarray[1]);
			...
			$totalresults = count($requestresult);
				...
				if($totalresults > "0")
				{
				$nextrequesturl = "reportcategorygroups?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
				$nextrequesttype = "GET";
				$nextrequestbody = "";
			...
			$PHPTemplateLayer->assignGlobal("pagenumber",$pagenumber);
				...
				$itemcount = 0;
					...
					$grouptitle = $categoryobject->name;
					$groupid = $categoryobject->id;

					$PHPTemplateLayer->assign("contentid",$groupid);
					$PHPTemplateLayer->assign("contenttitle",$grouptitle);
					}
				}
				else
				{
