...
///*****************************************************************************///
	...
	$PHPTemplateLayer->prepare($install_path."/admin/templates/subscriptions.htm");
...
///*****************************************************************************///
	...
	$baseurl = "subscriptions.php?tab=$tab&pagenumber=";
		...
		if($search != "")
		{
		$requesturl = "membershipsubscriptions/search/".$search."?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
		}
		else
		{
		$requesturl = "membershipsubscriptions?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
		}
		...
		$requestresult = json_decode($requestresultarray[1]);
			...
			$totalresults = count($requestresult);
				...
				if($totalresults > "0")
				{
					if($search != "")
					{
					$nextrequesturl = "membershipsubscriptions/search/".$search."?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
					}
					else
					{
					$nextrequesturl = "membershipsubscriptions?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
					}
