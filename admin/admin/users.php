...
///*****************************************************************************///
	...
	$PHPTemplateLayer->prepare($install_path."/admin/templates/users.htm");
...
///*****************************************************************************///
	...
	$baseurl = "users.php?tab=$tab&pagenumber=";
	...
	$requesturl = "users/admin?pageNumber=".$pagenumber."&pageSize=".ADMIN_PERPAGE;
	$requesttype = "GET";
	$requestbody = "";
	...
	$requestresult = process_connect_Curl($U_SESSION_API_TOKEN,$requesturl,$requesttype,$requestbody,API_URL,API_USERNAME,API_PASSWORD,API_USERAGENT,SETTING_LOGS,$U_DATE,$U_DATETIME,INSTALL_TYPE,INSTALL_VERSION);
		...
		$requestresult = json_decode($requestresultarray[1]);
			...
			$totalresults = count($requestresult);
				...
				if($totalresults > "0")
				{
				$nextrequesturl = "users/admin?pageNumber=".($pagenumber+1)."&pageSize=".ADMIN_PERPAGE;
				$nextrequesttype = "GET";
				$nextrequestbody = "";
			...
			$PHPTemplateLayer->assignGlobal("pagenumber",$pagenumber);
				...
				$itemcount = 0;
					...
					foreach($requestresult as $userkey => $userobject)
					{
					...
					$userfullname = $userobject->fullName;
						...
						{
						$userfullname = '[API Connection]';
						}

					$PHPTemplateLayer->assign("contentid",$userid);
					$PHPTemplateLayer->assign("contentusername",$userusername);
					$PHPTemplateLayer->assign("contentname",$userfullname);
					}
				}
				else
				{
...
///*****************************************************************************///
		...
		$PHPTemplateLayer->block("SUCCESS");
			...
			elseif($success == "4")
			{
			$success = 'You have successfully updated this user\'s password';
			}
